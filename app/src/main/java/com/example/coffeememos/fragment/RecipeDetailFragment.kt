...
class RecipeDetailFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.selectedBean.observe(viewLifecycleOwner) { bean ->
            binding.countryText.text = bean.country
            binding.farmText.text = bean.farm
            binding.districtText.text = bean.district
            binding.speciesText.text = bean.species
            binding.processText.text = Constants.processList[bean.process]
            binding.elevationText.text = getString(R.string.elevation_from_to, bean.elevationFrom.toString(), bean.elevationTo.toString())
            binding.storeText.text = bean.store
            binding.beanCommentText.text = bean.comment

            if (bean.isFavorite) binding.beanFavoriteIcon.setImageResource(R.drawable.ic_baseline_favorite_24)
            else binding.beanFavoriteIcon.setImageResource(R.drawable.ic_baseline_favorite_border_24)
        }
    }
